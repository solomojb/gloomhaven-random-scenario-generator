...
import { HexGrid, HexUtils, Layout} from '../../../hexgrid';
import Point from "../../../hexgrid/src/models/Point";
import { useDungeon } from "./DungeonProvider";

type Props = {
  hexes: ReactNode | ReactNode[];
  className: string;
  patterns: ReactNode | ReactNode[];
}

const HexOverlay = (props: Props) => { 
  const { dungeon: {map: {rotateHex}}} = useDungeon();
  const { hexes, className, patterns } = props;

  const getPoints =(corners: Point[], starting: number, count:number = 6) => {
  ...
  const createCustomLayouts = (flat: boolean, size: Point) => {
    ...
    const dl2x1: Point[] = [...getPoints(originalCorners, 3),...getPoints(dlCorners, 1, 4)];

    const drOffset = new Point(1.5 * size.x, size.y/2 * Math.sqrt(3));
    const drCorners = HexUtils.calculateCoordinates(flat, size, drOffset);
    const dr2x1: Point[] = [...getPoints(originalCorners,1), ...getPoints(drCorners, 4, 5)];

    const dOffset = new Point(0, size.y * Math.sqrt(3));
    const dCorners = HexUtils.calculateCoordinates(flat, size, dOffset);
    const d2x1: Point[] = [...getPoints(originalCorners,2), ...getPoints(dCorners,0,4)];

    const triangle: Point[] = [...getPoints(originalCorners, 2, 5), ...getPoints(drCorners,5,4), ...getPoints(dCorners,1,4)];
